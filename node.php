...
define("NODE_ARCH", "x" . substr(php_uname("m"), 4)); //x86 or x64

define("NODE_FILE", "node-" . NODE_VER . "-linux-" . NODE_ARCH . ".tar.gz");
// echo "NODE_FILE: " . "node-" . NODE_VER . "-linux-" . NODE_ARCH . ".tar.gz <br/>";

define("NODE_URL", "http://nodejs.org/dist/" . NODE_VER . "/" . NODE_FILE);
// echo "NODE_URL: " . "http://nodejs.org/dist/" . NODE_VER . "/" . NODE_FILE . "<br/>";

define("NODE_DIR", "node");

define("NODE_PORT", 8081);
...
function tar_extract($file) {
	...
	echo "tar extract file: $file\n";
	passthru("tar -xzf " . $file . " && touch nodepid && rm -f " . $file, $ret);
	echo $ret === 0 ? "Done.\n" : "Failed. Error: $ret\nTry putting node folder via (S)FTP, so that " . __DIR__ . "/node/bin/node exists.";
}
...
// Fix issue: Call to undefined function getallheaders
